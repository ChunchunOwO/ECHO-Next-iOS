...
- 网易云音乐：支持自定义 NeteaseCloudMusicApi、二维码登录、账号信息、歌单、搜索和播放。
...
- 外源数据：歌词优先使用 LRCLIB，封面优先使用网易云音乐；LrcAPI 可补充歌词、封面和艺术家。多个来源命中时可分别选择每个字段的来源。
...
- 网易云流媒体需要自行提供可访问的 NeteaseCloudMusicApi 服务。
...
- 设置项通过 App 本地个人数据保存，包括语言、默认页面、默认曲库、音频 tag、EQ、响度归一化、ECHO 连接开关和外源数据开关。
- 本地音乐状态保存在 `src/storage/localMusicStore.ts`，包括收藏、最近播放和本地队列。
- LRCLIB：优先用于获取歌曲歌词等。
- LrcAPI：可补充歌词、封面和艺术家。
- 网易云音乐：中文曲库补充，主要用于封面，也可作为歌词 fallback。
- 多个来源同时命中时，可以分别指定歌词、艺术家和封面使用哪个来源。
