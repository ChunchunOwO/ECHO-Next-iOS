...
export type EchoNativeAction = {
  ...
  index?: number;
  key?: string;
  mode?: 'local' | 'pc' | 'phone' | 'remoteControl' | 'remoteStream' | 'streaming';
  name?: string;
  ...
  source?: 'echo' | 'local' | 'remote' | 'streaming';
  scheme?: 'http' | 'https';
  text?: string;
  token?: string;
  trackId?: string;
...
export type EchoNativePlayerViewProps = SharedEqProps & {
  ...
  modeLabel: string;
  outputMode: 'local' | 'pc' | 'phone' | 'remoteControl' | 'remoteStream' | 'streaming';
  pagePayload: string;
  positionMs: number;
  queueCount: number;
  queuePayload: string;
